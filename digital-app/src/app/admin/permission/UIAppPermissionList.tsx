...
import { ExcludeRecordFilter } from 'core/widget/WEntityList';
...
export class UIPermissionList extends VGridEntityList<UIPermissionListProps> {
    ...
    let config: VGridConfig = {
      record: {
        fields: [
          ...
          { name: 'capability', label: T('Capability') },
  ...
  //Tofix
  // onDeleteAction() {
  //   const { plugin, appContext } = this.props;
  //   let itemId = plugin.getListModel().getSelectedRecordIds();
  //   if (itemId.length === 0) {
  //     widget.layout.showNotification('warning', T(`You need to select row to delete`));
  //     return;
  //   }
  ...
  //   appContext.serverDELETE(PermissionRestURL.permissions.delete, itemId, successCB);
  // }
  ...
  onChangeRole() {
    ...
    const onSelect = (role: any) => {
      ...
      const onCallBack = (result: server.rest.RestResponse) => {
  ...
  onAdd(capability: any) {
    let html = null;
    let { appContext, accessType } = this.props;
    ...
    //     <UIPartnerList
    //       type={'selector'}
    //       plugin={new UIPartnerListPlugin()}
    //       appContext={appContext} pageContext={popupPageCtx} readOnly={true}
    //       onSelect={(_appContext, pageCtx, employee) => onMultiSelect(pageCtx, [employee])}
    //       onMultiSelect={(_appContext, pageCtx, employees) => onMultiSelect(pageCtx, employees)} />
    //   );
    // } else {
